...
class Decoder(nn.Module):
    ...
    def forward(self,inputs,l_states,ast_enc,api_enc):
        config = self.config
        lengths_ast_enc = [x.shape[0] for x in ast_enc]
        ast_enc = rnn_utils.pad_sequence(ast_enc)
        ast_mask = [torch.ones(x).to(ast_enc.device) for x in lengths_ast_enc]
        ast_mask = rnn_utils.pad_sequence(ast_mask)
        ast_mask = ast_mask.unsqueeze(0)
        ast_mask = ast_mask.eq(1)

        lengths_api_enc = [x.shape[0] for x in api_enc]
        api_enc = rnn_utils.pad_sequence(api_enc)
        api_mask = [torch.ones(x).to(api_enc.device) for x in lengths_api_enc]
        ...
        ast_context = self.ast_attn(tensor,ast_enc,ast_enc,ast_mask)
        api_context = self.api_attn(tensor,api_enc,api_enc,api_mask)
        context = ast_context+api_context
...
class Actor(nn.Module):
    def __init__(self,config):
        ...
        self.config = config
        self.embedding_ast_enc = nn.Embedding(config.DICT_CODE,config.EMB_SIZE)
        self.embedding_api_enc = nn.Embedding(config.DICT_CODE,config.EMB_SIZE)
        self.embedding_dec = nn.Embedding(config.DICT_WORD,config.EMB_SIZE)
        self.dropout_ast_enc = nn.Dropout(config.DROPOUT)
        self.dropout_h = nn.Dropout(config.DROPOUT)
        self.dropout_c = nn.Dropout(config.DROPOUT)
        self.dropout_api_enc = nn.Dropout(config.DROPOUT)        
    ...
    def forward(self,inputs):
        device = self.device
        config = self.config
        ast_inputs = [torch.tensor(x).to(device) for x in inputs[0]]
        ast_inputs = [self.embedding_ast_enc(x) for x in ast_inputs]
        ast_enc,(h,c) = self.ast_encoder(ast_inputs)
        ast_enc = [self.dropout_ast_enc(x) for x in ast_enc]
        api_inputs = [torch.tensor(x).to(device) for x in inputs[1]]
        api_inputs = [self.embedding_api_enc(x) for x in api_inputs]
        api_enc,_ = self.api_encoder(api_inputs)
        api_enc = [self.dropout_api_enc(x) for x in api_enc]
        h = self.dropout_h(h)
        c = self.dropout_c(c)
        l_states = [(h,c) for _ in range(config.NUM_LAYER)]
        batch_size = len(ast_inputs)
        ...
        for i in range(config.MAX_COMMENT_LEN):
            ...
            tensor,l_states = self.decoder(tensor,l_states,ast_enc,api_enc)
...
class Critic(nn.Module):
    ...
    def forward(self,inputs,targets):
        ...
        api_inputs = [torch.tensor(x).to(device) for x in inputs[1]]
        api_inputs = [self.embedding_api_enc(x) for x in api_inputs]
        api_enc,(h,c) = self.api_encoder(api_inputs)
        api_enc = [self.dropout_api_enc(x) for x in api_enc]
        h = self.dropout_h(h)
        c = self.dropout_c(c)
        l_states = [(h,c) for _ in range(config.NUM_LAYER)]
...
class RLComAPI(nn.Module):
    def __init__(self,config):
        super(RLComAPI,self).__init__()
