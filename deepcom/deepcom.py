...
class Encoder(nn.Module):
    def __init__(self,config):
        ...
        for i in range(config.NUM_LAYER):
            ...
            nn.LSTM(config.ENC_SIZE,config.ENC_SIZE))
    ...
    def forward(self,inputs):
        ...
        inputs = rnn_utils.pad_sequence(inputs,padding_value=-1)
...
class Decoder(nn.Module):    
    def __init__(self,config):
        ...
        for i in range(config.NUM_LAYER-1):
            self.__setattr__("layer_{}".format(i),
            nn.LSTM(config.DEC_SIZE,config.DEC_SIZE))
    ...
    def translate(self,enc,states):
        ...
        for t in range(config.MAX_COMMENT_LEN):
